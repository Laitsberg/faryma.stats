...
test('у каждого разноса есть эфир и название', () => {
...
test('почти у всех разносов есть номер трека', () => {
  // Номер задаёт порядок внутри эфира — тайм-коды местами разъезжаются,
  // и сортировка идёт по нему. Но требовать его у каждой строки нельзя:
  // свежий эфир вносят в таблицу по ходу, и пара строк какое-то время
  // живёт без номера. Порог ловит поломку парсера, а не работу в
  // процессе: если номера перестанут разбираться, их не станет у сотен.
  const без = rows.filter(r => r.pos == null);
  assert.ok(без.length / rows.length < 0.01,
    `без номера ${без.length} из ${rows.length}: ` +
    без.slice(0, 3).map(r => r.title).join(' · '));
});
...
test('номера эфиров уникальны, и нулевой на месте', () => {
  ...
  /* Даты нет только у ещё не состоявшегося эфира: его анонсируют
     заранее и вместо числа пишут «ХХ.09.26». Таких может быть один, и
     это обязан быть самый свежий номер — иначе сломался разбор даты,
     а не таблица. Раньше здесь стоял ноль, и проверка краснела каждый
     раз, когда анонсировали следующий стрим. */
  const безДаты = streams.filter(s => !s.date);
  assert.ok(безДаты.length <= 1,
    `без даты сразу ${безДаты.length} эфиров: ` +
    безДаты.slice(0, 5).map(s => s.num).join(', ') + '…');
  if (безДаты.length)
    assert.equal(безДаты[0].num, Math.max(...nums),
      `без даты эфир №${безДаты[0].num}, а не самый свежий`);
});
