...
test('выбор равномерен и не любит первый сектор', async () => {
  const края = await page.evaluate(() => {
    ...
    for (let i = 0; i < 50000; i++) счёт[случайный(n)]++;
    const ожидание = 50000 / n;
    return { мин: Math.min(...счёт) / ожидание, макс: Math.max(...счёт) / ожидание };
  });
  assert.ok(края.мин > 0.9 && края.макс < 1.1,
    `перекос: от ${края.мин.toFixed(2)} до ${края.макс.toFixed(2)} от ожидаемого`);
});
