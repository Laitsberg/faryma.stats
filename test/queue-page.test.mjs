...
const ВИДЫ = [['донат', 4], ['заказ', 28], ['повтор', 1], ['долг ×9', 9],
  ...
  ['долг ×4', 11], ['долг ×3', 41], ['долг ×2', 30], ['долг ×1', 12]];
// «долг ×1» именно так и приходит: бот переводит в него голое «долг»
...
test('распорядок разбора собран из тех ступеней, что в очереди', async () => {
  // Верхняя ступень не постоянна: что не разобрали в субботу, поднимается
  // на ступень выше. Написанное руками «×8» в такой день начинает врать,
  // поэтому числа должны приходить из данных
  const итог = await page.evaluate(() => ({
    обычный: document.getElementById('qOrderNormal').textContent,
    долговой: document.getElementById('qOrderDebt').textContent
  }));
  const верх = Math.max(...ВИДЫ.map(([т]) => +(т.match(/×(\d+)/) || [])[1] || 0));
  assert.equal(верх, 9, 'подделка перестала содержать долг ×9 — проверка потеряла смысл');
  assert.match(итог.обычный, /долг ×9/, 'в обычном распорядке нет верхней ступени');
  assert.match(итог.долговой, /^Долг ×9/, 'долговой распорядок начинается не с верхней ступени');
  assert.match(итог.долговой, /снова с ×9/, 'круг замыкается не на верхней ступени');
  // ×8 как верхняя ступень — это ровно то, что было написано руками
  assert.ok(!/^Долг ×8/.test(итог.долговой), 'распорядок написан руками, а не собран');
  // ступень без числа печаталась как «долг ×null» — глазами видно, проверкой нет
  for (const [чей, строка] of [['обычном', итог.обычный], ['долговом', итог.долговой]])
    assert.ok(!/null|NaN|undefined/.test(строка),
      `в ${чей} распорядке ступень без числа: ${строка}`);
  assert.match(итог.долговой, /долг ×1 → и снова/, 'нижняя ступень показана неверно');
});
