...
test('полоса свежести показывает самый последний эфир', async () => {
  const итог = await page.evaluate(() => {
    ...
    return {
      ...
      естьПометка: /таблицу ещё заполняют/.test(текст),
      текстПолосы: текст
    };
  });
  ...
  if (итог.сЗаписью) {
    ...
    // про эфир сайт утверждать не вправе: идёт ли стрим, он не знает
    assert.ok(!/сейчас разбирают|эфир идёт/.test(итог.текстПолосы),
      'полоса утверждает то, чего сайт не знает');
  }
});
