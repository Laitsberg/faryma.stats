...
/* ---------- что было на последнем эфире ----------
   ...
   И говорим при этом про ТАБЛИЦУ, а не про эфир. Идёт ли стрим прямо
   сейчас, сайт не знает и знать не может — он видит одну таблицу.
   Прежняя формулировка про «разбор» читалась как «сейчас разбирают»,
   хотя эфир кончился накануне, а модератор просто проставляет
   тайм-коды. Про то, чего не знаешь, лучше молчать. */
/* Сколько строк последнего эфира видно на телефоне. Столько же
   прячет CSS: .last-row:nth-child(n+9). Меняешь тут — меняй там. */
const LAST_SHOW = 8;
...
function renderFresh() {
  ...
  /* Среднее считаем только у эфира с записью: пока таблицу заполняют,
     балл посчитан по случайной половине и назавтра будет другим. */
  ...
  /* Список — это не вывод, а перечисление внесённых строк, поэтому его
     можно показывать и у недозаполненного эфира: каждая строка тут
     уже правда, даже если завтра к ним добавятся ещё. */
  const список = [...rows].sort((a, b) => (a.pos || 0) - (b.pos || 0));
  ...
  el.innerHTML =
    ...
    `</div>` +
    `<div class="last-k">что принесли</div>` +
    `<div class="last-list">` + список.map(r =>
      `<div class="last-row">` +
        `<i>${r.pos ? esc(String(r.pos)) : '·'}</i>` +
        `<span>${artistNames(r.artist)} — <b>${esc(r.title)}</b></span>` +
        ratePill(r.rate.label) +
      `</div>`).join('') +
    `</div>` +
    /* На узком экране список не прокручивается внутри себя — прокрутка
       внутри прокрутки на телефоне мучительна. Вместо неё CSS прячет
       всё после восьмой строки, а эта ссылка ведёт в карточку стрима,
       где список целиком. */
    (список.length > LAST_SHOW
      ? `<a class="last-more" href="#stream=${st.num}" data-stream="${st.num}">` +
        `и ещё ${num(список.length - LAST_SHOW)} ` +
        plural(список.length - LAST_SHOW, 'трек', 'трека', 'треков') +
        ` — в карточке стрима</a>`
      : '');

  el.querySelector('.last-list')
    ?.addEventListener('scroll', отметитьХвост, { passive: true });
  равнятьЭфир();
}

/* Правая колонка равняется по левой.
   Содержимое у неё непредсказуемой длины: на одном эфире восемь треков,
   на другом шестьдесят. Какую сторону ни оставь свободной — рядом
   образуется пустота: сперва пустовало под карточкой, потом под пультом.
   Средствами разметки «будь ростом с соседом, но не выше» не выражается,
   поэтому высоту задаём отсюда, а список внутри прокручивается.
   На узком экране колонки идут друг под другом, и равнять нечего. */
function равнятьЭфир() {
  const пульт = document.querySelector('.desk'), карточка = $('fresh');
  if (!пульт || !карточка || карточка.hidden) return;
  карточка.style.height = window.innerWidth > 900 ? пульт.offsetHeight + 'px' : '';
  отметитьХвост();
}

/* Список кончился не там, где обрывается карточка: у нижнего края
   ложится тень. Без неё строка, обрезанная посередине, выглядит
   сломанной вёрсткой, а не продолжением. */
function отметитьХвост() {
  const карточка = $('fresh'), л = карточка?.querySelector('.last-list');
  if (!л) return;
  карточка.classList.toggle('more', л.scrollHeight - л.clientHeight - л.scrollTop > 8);
}

/* Пульт меняет рост сам по себе: плашки годов переносятся на вторую
   строку, шрифты догружаются позже разметки. Следим за ним, а не за
   окном: событие resize этих случаев не покрывает. */
if (typeof ResizeObserver !== 'undefined') {
  const пульт = () => document.querySelector('.desk');
  const наблюдатель = new ResizeObserver(() => равнятьЭфир());
  const запустить = () => { const d = пульт(); if (d) наблюдатель.observe(d); };
  document.readyState === 'loading'
    ? document.addEventListener('DOMContentLoaded', запустить)
    : запустить();
}
addEventListener('resize', () => равнятьЭфир());
