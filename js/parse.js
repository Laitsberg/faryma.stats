...
/* ---------- насколько строка подходит под запрос ----------
   Сайт и чат должны отвечать одинаково: человек спросил в чате,
   перешёл по ссылке — и видит сверху то же самое. Поэтому правило
   одно и лежит здесь, рядом с разбором строк, а не в двух копиях.

   Дело не в том, что находится, а в том, в каком порядке. Сайт
   показывал найденное по алфавиту, и на запрос «eve» исполнитель
   Eve оказывался пятьдесят третьим: выше стояли чужие треки и ники
   заказчиков, где «eve» попалось внутри слова — «StillReverie»,
   «@Forever_temochka». Двадцать две его записи приходилось искать
   глазами среди трёхсот.

   Сузить поиск до целого слова (так делает бот, ему нужен один
   ответ) здесь нельзя: «ева» перестанет находить «Королеву», а
   «lil» — «Lilith». Считано по архиву: у «ева» целым словом ноль
   совпадений против тридцати семи, у «lil» — четыре против
   двадцати девяти. Поэтому находим всё, но подходящее поднимаем.

   Чем меньше число, тем точнее попадание. */
function searchRanker(q) {
  const н = String(q || '').trim().toLowerCase();
  if (!н) return null;

  const сжать = s => String(s || '').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '');
  const сж = сжать(н);

  /* Регулярку собираем один раз на запрос, а не на каждую строку:
     в архиве почти семь тысяч записей, и поиск считается на каждую
     набранную букву. */
  const сНачалаСлова = new RegExp(
    '(^|[^\\p{L}\\p{N}])' + н.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'u');

  return (artist, title, user) => {
    const t = String(title  || '').toLowerCase();
    const a = String(artist || '').toLowerCase();
    const u = String(user   || '').toLowerCase();

    /* Точное совпадение с названием — самое сильное: на «unravel»
       человек ждёт «Unravel», а не «Unravelling». Дальше имя весит
       больше названия: набирают чаще исполнителя, а внутри чужого
       длинного названия запрос попадается случайно. Считано по
       архиву: «ева» находит «танц-ева-ть» и «изменени-ева»
       вперемешку с «Пугач-ева» и «Рымба-ева», и артисты должны
       стоять выше. */
    if (t === н || (сж && сжать(t) === сж)) return 0;
    if (a === н || (сж && сжать(a) === сж)) return 1;
    // «Lil Peep» выше, чем «Lilith», а «Lilith» — выше заказчика
    // «AnonymousAdomin»: начало слова весит больше середины
    if (сНачалаСлова.test(a)) return 2;
    if (t.startsWith(н)) return 3;
    if (a.includes(н) || (сж && сжать(a).includes(сж))) return 4;
    if (t.includes(н) || (сж && сжать(t).includes(сж))) return 5;
    if (u.includes(н)) return 6;
    return 7;   // совпало где-то ещё: в источнике, в скобках, в сжатом виде
  };
}
