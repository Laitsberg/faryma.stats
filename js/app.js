...
function renderSearch() {
  ...
  $('searchCount').textContent = `найдено ${num(rows.length)}` +
    (q ? ' · сначала самые подходящие' : '') +
    (rows.length > SEARCH_LIMIT ? ` · показаны первые ${SEARCH_LIMIT}` : '');
  отборВАдрес();

  /* Порядок по алфавиту хорош, когда запроса нет: листаешь архив.
     Но стоит что-то набрать — и он вредит: на «eve» исполнитель Eve
     уходил на пятьдесят третье место, потому что выше по алфавиту
     стояли чужие треки, где «eve» нашлось внутри ника заказчика.
     Поэтому при новом запросе возвращаем порядок «сначала подходящее».

     Нажатие на заголовок сильнее и держится, пока запрос не поменяли:
     dataset.q помнит, к какому запросу относится выбранная колонка. */
  const табл = $('tSearch');
  if (табл && табл.dataset.q !== q) {
    табл.dataset.q = q;
    табл.dataset.sort = q ? 'rel' : 'artist';
    // явно по возрастанию: ноль — самое точное попадание, а числовым
    // колонкам таблица по умолчанию даёт обратный порядок
    табл.dataset.dir = 'asc';
  }
  ...
  /* Оценку попадания считает общая с ботом функция: спросишь в чате
     и на сайте — порядок совпадёт. */
  const ранг = searchRanker(q);

  const view = rows
    .map(r => ({
      // Равные по точности остаются в порядке архива, то есть от новых
      // к старым: сортировка в table() устойчива и своих не переставляет
      rel: ранг ? ранг(r.artist, r.title, r.user) : 0,
  ...
  table('tSearch', [
    /* Скрытая колонка: точность совпадения в таблице не нужна, но
       именно она решает порядок, и на телефоне её надо назвать —
       иначе список сортировки уверяет, что сортирует по алфавиту,
       а список идёт совсем не так. */
    ...(q ? [{ k: 'rel', t: 'сначала подходящие', hidden: 1 }] : []),
  ...
  ], view, q ? 'rel' : 'artist', SEARCH_LIMIT);
}
