...
const todo = tracks.filter(t => {
  ...
  /* Кого ссылка привела на переиздание. Судим по тому, что уже
     записано в кэше: второй раз ходить в Spotify, чтобы узнать, стоит
     ли туда идти, было бы странно. Пометка «переиздание» появилась
     вместе с этими правилами, поэтому смотрим и её, и сохранённые
     названия — у прежних записей пометки ещё нет.
     Только тех, у кого в таблице есть ссылка: у найденных поиском
     правила не менялись, и переспрашивать их — впустую тратить
     запросы. */
  if (RECHECK_REISSUES && t.sid && (hit.v ?? 1) < ВЕРСИЯ &&
      (hit.переиздание || переиздание(hit.spTitle, hit.spAlbum))) return true;
  return false;
}).slice(0, LIMIT);
