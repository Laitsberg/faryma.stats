...
/* ============================================================
   ...
   продолжит с того же места. Уже известных не переспрашивает.
     ...
     node scripts/years.mjs --recheck        переспросить после починки
                                             правил совпадения
     node scripts/years.mjs --recheck-reissues
                                             пересверить тех, чья
                                             ссылка ведёт на концертник
                                             или ремастер
...
import { переиздание } from './reissue.mjs';
...
/* Переспросить только тех, у кого ссылка привела на переиздание:
   концертник, ремастер, юбилейный сборник. Их в кэше пара десятков,
   а --recheck переспрашивает все полторы тысячи и выбирает лимит
   Spotify досуха ради полусотни исправлений. Имя флага латиницей:
   кириллицу не принимают ни bash, ни выражения GitHub Actions. */
const RECHECK_REISSUES = hasFlag('--recheck-reissues');
...
async function spGet(path, attempt = 0) {
  ...
  try {
    ...
    /* Причину пишем словами. Раньше здесь стояло одно «не ответил за
       15 с», и любая другая беда — кривой заголовок, сбитый адрес —
       выглядела в логе таймаутом, которого не было. */
    const почему = e.name === 'TimeoutError' || e.name === 'AbortError'
      ? 'не ответил за 15 с' : `ошибка запроса (${e.name}: ${e.message})`;
    throw new Error(`Spotify ${почему} на ${path.slice(0, 60)}`);
  }
...
/* Обложка приезжает в том же ответе, что и год, — лишних запросов не
...
async function spПоСсылкам(список, cache, закрыто, startedAt, maxMs) {
  ...
  for (const t of список) {
    ...
    /* Ссылка сама по себе год не решает: в таблице она порой ведёт на
       концертник, ремастер или юбилейный сборник, вышедший много позже
       песни. У «Ado — Basket Worm» так получился 2026-й с альбома «Ado
       1st Live Kigeki (Live At Zepp DiverCity, 2022)», у «Led Zeppelin
       — No Quarter» 2007-й с «Mothership (Remastered)».

       Такие сверяем поиском всегда, не дожидаясь --crosscheck. Сверять
       всех подряд мы себе позволить не можем — это вдвое больше
       запросов, а лимит Spotify выбирался досуха, — но переизданий в
       архиве три-четыре десятка на полторы тысячи, и лишние запросы на
       них теряются в шуме. */
    const подозрение = tr ? переиздание(tr.name, tr.album && tr.album.name) : false;
    let поиском = null, сверяли = false;
    if (tr && (СВЕРЯТЬ || подозрение)) {
      сверяли = true;
      await sleep(SP_DELAY);
      /* Глубже листаем только подозрительных: у обычной ссылки год и
         так верен, а лишние страницы — лишние запросы. */
      поиском = await spПоиском(t, подозрение).catch(() => null);
    }
    const года = [годСсылки, поиском && поиском.year].filter(Boolean);
    /* Год переиздания — не год песни. Если поиск оригинала не нашёл,
       честнее остаться совсем без года, чем показать зрителю 2007-й у
       песни 1973 года. Обложка, альбом и длительность при этом верны:
       они описывают ту самую запись, на которую ведёт ссылка, — и
       страница их покажет, там свой порог доверия, по «как». */
    const year = (подозрение && !(поиском && поиском.year))
      ? null
      : (года.length ? Math.min(...года) : null);
    ...
    if (tr) {
      ...
      cache.tracks[t.key] = {
        ...
        /* Пишем пометку, а не вычисляем на лету: по ней видно в файле,
           почему у трека с точной ссылкой нет года, и по ней же
           --recheck-reissues отбирает, кого пересверить. */
        ...(подозрение ? { переиздание: true } : {})
      };
      if (year) { найдено++; закрыто.add(t.key); }
      /* Сверенного поиском в общий проход не отдаём, даже когда года
         так и нет: тот же запрос ничего нового не скажет, а запись,
         которую он перезапишет, потеряет обложку и альбом от ссылки. */
      else if (сверяли) закрыто.add(t.key);
    }
    сделано++;
    if (сделано % 50 === 0) {
...
/* Сколько страниц выдачи листать. limit=10 — потолок этого приложения:
   пробой проверено, что 20 и 50 отвергаются с «Invalid limit», а 10
   отдаётся. Одной страницы обычно хватает: поиск сортирует по
   релевантности, и оригинал чаще всего в первой десятке.

   Чаще всего — но не когда песню переиздавали. Первая пересверка это
   и показала: у «Kenny Loggins — Danger Zone» в десятке одни сборники
   и концертники, у «BT» и «Bear McCreary» — только ремастеры, и год
   оригинала (1986-й и 2006-й) в неё просто не попал. Поэтому для
   подозрительных листаем три страницы: тридцать кандидатов вместо
   десяти, два лишних запроса на трек. Переизданий в архиве три-четыре
   десятка, так что для лимита Spotify это ничто. */
const СТРАНИЦ_ОБЫЧНО = 1;
const СТРАНИЦ_ГЛУБЖЕ = 3;

async function spПоиском(t, глубже = false) {
  const q = `track:"${t.title.replace(/"/g, ' ')}" artist:"${queryName(t.artist).replace(/"/g, ' ')}"`;
  const страниц = глубже ? СТРАНИЦ_ГЛУБЖЕ : СТРАНИЦ_ОБЫЧНО;
  const найденные = [];
  for (let с = 0; с < страниц; с++) {
    const j = await spGet(`/search?type=track&limit=10&offset=${с * 10}&q=` + encodeURIComponent(q));
    const порция = j.tracks?.items || [];
    найденные.push(...порция);
    // Страница пришла неполной — дальше ничего нет, незачем спрашивать.
    if (порция.length < 10) break;
    if (с + 1 < страниц) await sleep(SP_DELAY);
  }
  const все = [];
  const видели = new Set();
  for (const tr of найденные) {
    // Страницы иногда перекрываются: один и тот же трек дважды в
    // выборе самого раннего ничего не меняет, но путает счётчик
    // «вариантов», по которому потом смотрят, много ли нашлось.
    if (!tr || видели.has(tr.id)) continue;
    видели.add(tr.id);
...
const todo = tracks.filter(t => {
  ...
  /* Пересверка переизданий — проход ЦЕЛЕВОЙ: только те, чья ссылка
     привела на концертник, ремастер или юбилейный сборник, и больше
     никто. Сначала было «вдобавок к обычной докачке», и толку от
     флага не вышло: незнакомых песен в архиве пять тысяч, они и
     забрали весь лимит, а до трёх десятков переизданий дело не
     дошло. Кого сверять, решаем по тому, что уже записано в кэше:
     второй раз ходить в Spotify, чтобы узнать, стоит ли туда идти,
     было бы странно. Пометка «переиздание» появилась вместе с этими
     правилами, поэтому смотрим и её, и сохранённые названия — у
     прежних записей пометки ещё нет. И только те, у кого в таблице
     есть ссылка: у найденных поиском правила не менялись. */
  if (RECHECK_REISSUES)
    return !!hit && !!t.sid && (hit.v ?? 1) < ВЕРСИЯ &&
           (hit.переиздание || переиздание(hit.spTitle, hit.spAlbum));
